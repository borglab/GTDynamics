...
gtsam::NonlinearFactorGraph DynamicsGraphBuilder::qFactors(const UniversalRobot &robot, const int t) const
{
...
gtsam::NonlinearFactorGraph DynamicsGraphBuilder::vFactors(const UniversalRobot &robot, const int t) const
{
    NonlinearFactorGraph graph;
    for (auto &&link : robot.links())
    {
...
gtsam::NonlinearFactorGraph DynamicsGraphBuilder::aFactors(const UniversalRobot &robot, const int t) const
{
    NonlinearFactorGraph graph;
    for (auto &&link : robot.links())
    {
...
gtsam::NonlinearFactorGraph DynamicsGraphBuilder::dynamicsFactors(const UniversalRobot &robot, const int t,
    ...
    const boost::optional<gtsam::Vector3> &planar_axis) const
{
    ...
    {
        // in-phase
        for (int phase_step = 0; phase_step < phase_steps[phase] - 1; phase_step++)
        {
    ...
    for (int phase = 0; phase < num_phases; phase++)
    {
        ...
        {
            graph.add(multiPhaseCollocationFactors(robots[phase], t++, phase, collocation));
        }
    }
    return graph;
}
...
gtsam::NonlinearFactorGraph DynamicsGraphBuilder::multiPhaseCollocationFactors(const UniversalRobot &robot,
