...
import numpy as np
import sympy
from sympy import Matrix, N, S, Symbol, solve_poly_system, solveset
...
class FasterIK:
    ...
    def Z(label: str = ""):
        ...
        return Matrix([[c, -s, 0], [s, c, 0], [0, 0, 1]]), c**2 + s**2 - 1, [c, s]

    @classmethod
    def RRR_rotation(cls, R12, R23, R3e, label: str = ""):
        ...
        """
        Z1, e1, syms1 = cls.Z(label+"1")
        Z2, e2, syms2 = cls.Z(label+"2")
        Z3, e3, syms3 = cls.Z(label+"3")
        syms = syms1 + syms2 + syms3
        return Z1 * R12 * Z2 * R23 * Z3 * R3e, [Z1, Z2, Z3], [e1, e2, e3], syms

    @classmethod
    def RRR_pose(cls, R12, t12, R23, t23, R3e, t3e, label: str = ""):
        ...
        """
        Re, Zs, constraints, syms = cls.RRR_rotation(R12, R23, R3e, label)
        Z1, Z2, Z3 = Zs
    ...
    def RRR_solve(cls, R12, t12, R23, t23, R3e, t3e, lock_theta1=False, label: str = ""):
        ...
        if lock_theta1:
            ...
            c1, s1 = syms[0:2]
            syms = syms[2:]
            constraints = [c.subs([(c1, 1), (s1, 0)]) for c in constraints[1:]]
...
if __name__ == "__main__":
    ...
    solutions, syms = FasterIK.RRR_solve(
        *TestInverseKinematics.ROBOT, lock_theta1=True)
    end = time.time()
    print("elapsed time = {} secs", end - start)
