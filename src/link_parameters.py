...
RR_calibration_dh = 2 * \
    [DH_Link(0, 0, 2, 0, 'R', 1, Point3(-1, 0, 0), np.diag([0, 0, 0]))]

# Denavit-Hartenberg parameters for PUMA manipulator theta, d, a, alpha
PUMA_calibration_dh = [
    DH_Link(0, 0.0000, 0.0000, +90, 'R', 0, Point3(0, 0, 0), np.diag([0, 0.35, 0])),
...
RR_calibration_urdf = [
    URDF_Link(Pose3(Rot3(), Point3(2, 0, 0)), utils.vector(0, 0, 1), 'R', 1, Pose3(Rot3(), Point3(1, 0, 0)), np.diag([0, 0, 0])),
    URDF_Link(Pose3(Rot3(), Point3(2, 0, 0)), utils.vector(0, 0, 1), 'R', 1, Pose3(Rot3(), Point3(1, 0, 0)), np.diag([0, 0, 0]))
]
