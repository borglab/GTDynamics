...
/**
 * @file  testWalkForward.cpp
 * @brief test of Quadruped trajectory optimization with phases.
 * @Author: Alejandro Escontrela, Varun Agrwawal, Stefanos
 */
...
TEST(walkForward, WholeEnchilada) {
  // Load vision 60 quadruped by Ghost robotics (https://youtu.be/wrBNJKZKg10).
  Robot vision60 =
      gtdynamics::CreateRobotFromFile(URDF_PATH + "/vision60.urdf");
  ...
  // All contacts.
  // TODO(frank): #144 figure out why Linter complains
  using CP = ContactPoint;
  CP c0 = {Point3(0.14, 0, 0), 0, GROUND_HEIGHT};  // Front left
  CP c1 = {Point3(0.14, 0, 0), 0, GROUND_HEIGHT};  // Hind  left
  CP c2 = {Point3(0.14, 0, 0), 0, GROUND_HEIGHT};  // Front right.
  CP c3 = {Point3(0.14, 0, 0), 0, GROUND_HEIGHT};  // Hind right.
  ...
  CPs p0 = {{"lower0", c0},
            {"lower1", c1},
            {"lower2", c2},
            {"lower3", c3}};  // Initially stationary.
  CPs t01 = {{"lower1", c1}, {"lower2", c2}, {"lower3", c3}};
  CPs p1 = {{"lower1", c1},
            {"lower2", c2},
            {"lower3", c3}};  // Front right leg swing.
  CPs t12 = {{"lower2", c2}, {"lower3", c3}};
  CPs p2 = {
      {"lower0", c0}, {"lower2", c2}, {"lower3", c3}};  // Front left leg swing.
  CPs t23 = {{"lower0", c0}, {"lower3", c3}};
  CPs p3 = {
      {"lower0", c0}, {"lower1", c1}, {"lower3", c3}};  // Hind right swing.
  CPs t34 = {{"lower0", c0}, {"lower1", c1}};
  CPs p4 = {
      {"lower0", c0}, {"lower1", c1}, {"lower2", c2}};  // Hind left swing.
  CPs t45 = {{"lower1", c1}, {"lower2", c2}};
  CPs p5 = {{"lower1", c1},
            {"lower2", c2}};  // Front left leg and hind right swing.
  CPs t56 = {};
  CPs p6 = {{"lower0", c0},
            {"lower3", c3}};  // Front right leg and hind left swing.
  ...
  //         link, (link_map[link]->wTcom() * Pose3(Rot3(), c0.contact_point))
  //                   .translation()));
  ...
  //     for (auto&& cp : phase_cps[p]) phase_contact_links.push_back(cp.name);
  ...
  //             Pose3(Rot3(), c0.contact_point),
  //             Point3(prev_cp[pcl].x(), prev_cp[pcl].y(), GROUND_HEIGHT -
  //             0.03)));

  //       double h = GROUND_HEIGHT +
  //                  0.05 * std::pow(t_normed, 1.1) * std::pow(1 - t_normed,
  //                  0.7);
  ...
  //     objective_factors.add(gtsam::PriorFactor<gtsam::Pose3>(
  //         internal::PoseKey(link->id(), 0), link->wTcom(),
  //         dynamics_model_6));
  //     objective_factors.add(gtsam::PriorFactor<Vector6>(
  //         internal::TwistKey(link->id(), 0), Vector6::Zero(),
  //         dynamics_model_6));
  ...
  //     objective_factors.add(
  //         gtsam::PriorFactor<Vector6>(internal::TwistAccelKey(link->id(),
  //         t_f),
  //                                     Vector6::Zero(), objectives_model_6));
  //   }
  ...
  //     objective_factors.add(gtsam::PriorFactor<double>(
  //         internal::JointAccelKey(joint->id(), t_f), 0.0,
  //         objectives_model_1));
  //   }

  //   // Add prior factor constraining all Phase keys to have duration of 1 /
  //   240. for (int phase = 0; phase < phase_steps.size(); phase++)
  //     objective_factors.add(gtsam::PriorFactor<double>(
  ...
  //   // Log the joint angles, velocities, accels, torques, and current goal
  //   pose. vector<string> jnames; for (auto&& joint : vision60.joints())
  //   jnames.push_back(joint->name()); string jnames_str =
  //   boost::algorithm::join(jnames, ","); std::ofstream traj_file;
  ...
  //       vals.push_back(
  //           std::to_string(results.atDouble(internal::PhaseKey(phase))));
