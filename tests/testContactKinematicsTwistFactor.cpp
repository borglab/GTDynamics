...
TEST(ContactKinematicsTwistFactor, error) {
  ...
  // A link with zero linear/angular velocity at its CoM should have a stationary
  // contact point.
  gtsam::Vector6 link_twist_stationary = (
      gtsam::Vector(6) << 0, 0, 0, 0, 0, 0).finished();
  EXPECT(assert_equal(
      factor.evaluateError(link_twist_stationary),
  ...
  EXPECT(assert_equal(
      factor.evaluateError(link_twist_linear),
  ...
  EXPECT(assert_equal(
      gtsam::norm_2(factor.evaluateError(link_twist_angular)),
      1.0
  ));

  // A link with both angular velocity and linear velocity at the CoM should have
  // a linear velocity at the contact point (unless they cancel each other out).
...
TEST(ContactKinematicsTwistFactor, optimization) {
  ...
  EXPECT(assert_equal(
      factor.evaluateError(twist_optimized),
      (gtsam::Vector(3) << 0, 0, 0).finished()
  ));
}
