...
TEST(BarrierFactor, negative) {
  ...
  // Test constructor.
  BarrierFactor barrier_factor(base_factor, false);
  ...
  // Check error and jacobian.
  {
    ...
    EXPECT(assert_equal(expected_error1, barrier_factor.unwhitenedError(values)));
    EXPECT_CORRECT_FACTOR_JACOBIANS(barrier_factor, values, 1e-7, 1e-5);
  }
...
TEST(BarrierFactor, positive) {
  ...
  // Test constructor.
  BarrierFactor barrier_factor(base_factor, true);
