...
TEST(Simulate, simple_urdf) {
  ...
  int t = 1;
  JointValues qs = DynamicsGraph::jointAngles(my_robot, results, t);
  JointValues vs = DynamicsGraph::jointVels(my_robot, results, t);
  JointValues as = DynamicsGraph::jointAccels(my_robot, results, t);
  ...
  EXPECT(assert_equal(expected_qAngle,
                      qs.at<double>(my_robot.getJointByName("j1")->getKey())));
  EXPECT(assert_equal(expected_qVel,
                      vs.at<double>(my_robot.getJointByName("j1")->getKey())));
  EXPECT(assert_equal(expected_qAccel,
                      as.at<double>(my_robot.getJointByName("j1")->getKey())));
}
