...
TEST(testSpiderWalking, WholeEnchilada) {
  ...
  Robot spider =
      gtdynamics::CreateRobotFromFile(SDF_PATH + "/spider_alt.sdf", "spider");
  spider.print();
  ...
  //       double h =
  //           GROUND_HEIGHT + std::pow(t_normed, 1.1) * std::pow(1 - t_normed,
  //           0.7);
  ...
  //   240. for (int phase = 0; phase < spider_trajectory.numPhases(); phase++)
  //     objective_factors.add(gtsam::PriorFactor<double>(
