...
TEST(MultiJacobian, Add_Mult) {
  ...
  Matrix m = gtsam::Matrix2::Identity() * 2;
...
TEST(MultiJacobians, Mult) {
  ...
  jac1_x1.insert({x1, gtsam::Matrix2::Identity()});
  jac1_x2.insert({x2, gtsam::Matrix2::Identity()});
  jac1_x3.insert({x3, gtsam::Matrix2::Identity()});
  ...
  jac2_x1.insert({x1, gtsam::Matrix2::Identity()});
  jac2_x2.insert({x2, gtsam::Matrix2::Identity()});
  jac2_x3.insert({x1, 2*gtsam::Matrix2::Identity()});
  jac2_x3.insert({x2, -1*gtsam::Matrix2::Identity()});
  ...
  expected_jac_x1.insert({x1, gtsam::Matrix2::Identity()});
  expected_jac_x2.insert({x2, gtsam::Matrix2::Identity()});
  expected_jac_x3.insert({x1, 2*gtsam::Matrix2::Identity()});
  expected_jac_x3.insert({x2, -1*gtsam::Matrix2::Identity()});
...
TEST(MultiJacobian, ComputeBayesNetJacobian) {
  ...
  graph.add(JacobianFactor(x1, gtsam::Matrix1::Identity(), x2, gtsam::Matrix1::Identity(), x3, -gtsam::Matrix1::Identity(), Vector1(0), model1));
  Matrix21 H_3, H_4;
  H_3 << 1, 0;
  H_4 << 0, 1;
  graph.add(JacobianFactor(x3, H_3, x4, H_4, x5, -gtsam::Matrix2::Identity(), Vector2(0, 0), model2));
  ...
  jacobian_x3.addJacobian(x1, gtsam::Matrix1::Identity());
  jacobian_x3.addJacobian(x2, gtsam::Matrix1::Identity());
