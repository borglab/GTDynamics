...
IEVision60Robot GetVision60() {
  ...
  for (const auto& hip_joint : IEVision60Robot::hip_joint_names) {
    joint_lower_limits.insert({hip_joint, hip_joint_lower_limit});
    joint_upper_limits.insert({hip_joint, hip_joint_upper_limit});
  }
  ...
  for (const auto& hip_joint : IEVision60Robot::upper_joint_names) {
    torque_lower_limits.insert({hip_joint, upper_torque_lower_limit});
    torque_upper_limits.insert({hip_joint, upper_torque_upper_limit});
  }
...
TEST(IEVision60Robot, manifold) {
  ...
  /// Test retractor in the case that no constraints are violated
  {
    ...
    Vector xi = (Vector(24) << delta_A, delta_V, delta_P, delta_CR).finished();
  ...
  /// Test retractor in the case of violating torque limits
  {
    ...
    expected_active_indices.insert(14);
    EXPECT(assert_container_equality(expected_active_indices, new_manifold.activeIndices()));
    ...
    EXPECT(assert_equal(e_constraints.evaluateViolationL2Norm(new_manifold.values()), 0.0));
    EXPECT(assert_equal(i_constraints.evaluateViolationL2Norm(new_manifold.values()), 0.0));
    // for (const auto& joint: vision60.robot.joints()) {
    //   int j = joint->id();
