...
TEST(ManifoldOptimization, Rot2) {
  Key rot_key = 1;
...
TEST(ManifoldOptimizerType1_infeasible, so2) {
  ...
  auto mopt_params = boost::make_shared<ManifoldOptimizer::Params>();
  mopt_params->cc_params->lm_params.setMaxIterations(4);
  ManifoldOptimizerType1 optimizer(*costs, *constraints, init_values, lm_params,
                                   mopt_params);
  auto result = optimizer.optimize();
  // result.print();
...
TEST(ManifoldOptimizer, GaussNewtonEquality) {
  ...
  GaussNewtonOptimizer optimizer_m(graph_rot2, init_values_rot2, params);
  ManifoldOptimizerType1 optimizer_type1(*costs_cm, *constraints_cm,
                                         init_values_cm, params);
