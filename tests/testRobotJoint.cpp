...
TEST(RobotJoint, urdf_constructor) {
  ...
  RobotJointSharedPtr j1 = std::make_shared<RobotJoint>(
      RobotJoint(*simple_urdf.JointByName("j1"), j1_params.jointEffortType,
                 ...
                 j1_params.torqueLimitThreshold, l1, l2_weak));
  ...
  EXPECT(assert_equal(j1->screwAxis(), screw_axis_j1));

  // Test that parent to child link transform is correct for -pi/2 and pi/2.
  EXPECT(assert_equal(Pose3(Rot3::Rx(-M_PI / 2), Point3(0, 1, 1)),
                      j1->MpcCom(-M_PI / 2)));
  EXPECT(assert_equal(Pose3(Rot3::Rx(M_PI / 2), Point3(0, -1, 1)),
                      j1->MpcCom(M_PI / 2)));
...
TEST(RobotJoint, sdf_constructor) {
  ...
  EXPECT(assert_equal((gtsam::Vector(3) << 0, 0, 1).finished(), j1->axis()));
  ...
  // Check that the axis is correctly defined in the joint frame.
  EXPECT(assert_equal((gtsam::Vector(3) << 0, 1, 0).finished(), j2->axis()));
  ...
  EXPECT(assert_equal(gtsam::Pose3(gtsam::Rot3::Ry(M_PI / 4),
