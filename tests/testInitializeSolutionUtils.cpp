...
TEST(initialize_solution_utils, initialize_solution_inverse_kinematics) {
  ...
  EXPECT(assert_equal(
    0.0,
    (init_vals.at(gtdynamics::PoseKey(l1->getID(), 0)).cast<gtsam::Pose3>() * oTc_l1).translation().z()
  ))
  EXPECT(assert_equal(
    0.0,
    init_vals.atDouble(gtdynamics::JointAngleKey(my_robot.getJointByName("j1")->getID(), 0))
  ));

  // TODO(aescontrela): These unit tests should work once the issue with the ContactKinematicsPoseFactor is fixed.
//   EXPECT(assert_equal(
//       wTb_t[0],
//       init_vals.at(gtdynamics::PoseKey(l2->getID(), std::roundl(ts[0] / dt))).cast<gtsam::Pose3>())
//   );
//   EXPECT(assert_equal(
//     0.0,
//     (init_vals.at(gtdynamics::PoseKey(l1->getID(), std::roundl(ts[0] / dt))).cast<gtsam::Pose3>() * oTc_l1).translation().z(),
//     1e-3
//   ));
...
TEST(initialize_solution_utils, initialize_solution_zero_values) {
  ...
  gtsam::Values init_vals = gtdynamics::zero_values(my_robot, 0, contact_points);

  for (auto&& link : my_robot.links())
    EXPECT(assert_equal(
      link->wTcom(),
      init_vals.at(gtdynamics::PoseKey(link->getID(), 0)).cast<gtsam::Pose3>()
    ));

  for (auto&& joint : my_robot.joints())
    EXPECT(assert_equal(0.0, init_vals.atDouble(gtdynamics::JointAngleKey(joint->getID(), 0))));
}
...
TEST(initialize_solution_utils, initialize_solution_zero_values_trajectory) {
  ...
  gtsam::Values init_vals = gtdynamics::zero_values_trajectory(my_robot, 100, -1, contact_points);

  for (int t = 0; t <= 100; t++) {
    for (auto&& link : my_robot.links())
      EXPECT(assert_equal(
        link->wTcom(),
        init_vals.at(gtdynamics::PoseKey(link->getID(), t)).cast<gtsam::Pose3>()
      ));

    for (auto&& joint : my_robot.joints())
      EXPECT(assert_equal(0.0, init_vals.atDouble(gtdynamics::JointAngleKey(joint->getID(), t))));
  }
