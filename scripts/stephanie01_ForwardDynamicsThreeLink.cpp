...
TEST(DynamicsGraph, optimization) {
    ...
    using namespace three_link;

    // Build the factor graph for the robot.
    gtsam::Vector3 gravity = (gtsam::Vector(3) << 0, 0, -9.8).finished();   // NOTE: both of these were already set in RobotModels.h
    ...
    gtsam::Vector joint_angles = (gtsam::Vector(1) << 0).finished();         // NOTE: both of these were already set in RobotModels.h
    gtsam::Vector joint_vels = (gtsam::Vector(1) << 0).finished();

    gtsam::Vector joint_torques = (gtsam::Vector(1) << 0).finished();
    gtsam::NonlinearFactorGraph prior_factors = graph_builder.forwardDynamicsPriors(
    ...
    gtsam::Values result = graph_builder.optimize(
        graph, init_values, robot::DynamicsGraphBuilder::OptimizerType::GaussNewton);
