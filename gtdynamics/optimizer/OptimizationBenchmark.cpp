...
void PrintLatex(std::ostream& latex_os, std::string exp_name, size_t f_dim, size_t v_dim, double time, size_t num_iters, double constraint_vio, double cost) {
  boost::format scinotation("%.2e");
  latex_os << "& " + exp_name + " & $" << f_dim << " \\times "
           << v_dim << "$ & " << boost::format("%0.4f") % time << " & " << num_iters << " & "
           << scinotation % constraint_vio << " & " << boost::format("%4.2f") % cost << "\\\\\n";
}
...
Values OptimizeConstraintManifold(const EqConsOptProblem& problem,
  ...
  PrintLatex(latex_os, "\\textbf{" + exp_name + "}",
             problem_dim.first, problem_dim.second,
             optimization_time,
