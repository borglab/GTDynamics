...
class ScrewJointBase : public JointTyped<double, double> {
 ...
 public:
  ...
  ScrewJointBase(const sdf::Joint &sdf_joint, gtsam::Vector6 jScrewAxis,
                 const gtdynamics::JointParams &jps, LinkSharedPtr parent_link,
                 LinkSharedPtr child_link)
  ...
  Pose3 transformFromImpl(const LinkSharedPtr link,
  ...
  gtsam::Vector6 transformTwistFromImpl(
    ...
    return transformFromImpl(link, q_).AdjointMap() * this_twist_ +
