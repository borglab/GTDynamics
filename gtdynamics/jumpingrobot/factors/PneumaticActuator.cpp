...
gtsam::NonlinearFactorGraph PneumaticActuator::actuatorFactorGraph(const int t) const {
  ...
  gtsam::Key q_key = JointAngleKey(params_.j, t);   // joint angle
  gtsam::Key v_key = JointVelKey(params_.j, t);     // joint velocity
  gtsam::Key torque_key = TorqueKey(params_.j, t);  // torque
  ...
  graph.add(SmoothActuatorFactor(x_key, P_key, f_key, force_cost_model,
                                  params_.x0_coeffs, params_.k_coeffs, params_.f0_coeffs));
  ...
  if (params_.positive) {
    gtsam::ExpressionFactor<double> balance_factor(
        ...
        params_.kt * cm_to_m * x_expr - f_expr - params_.kt * params_.r * q_expr);
  ...
  graph.add(JointTorqueFactor(q_key, v_key, f_key, torque_key,
                              torque_cost_model, params_.q_anta_limit, params_.ka, params_.r, params_.b,
                              params_.positive));

  graph.add(GassLawFactor(P_key, vo_key, m_key, gass_law_model,
                          params_.Rs * params_.T));

  graph.add(ActuatorVolumeFactor(vo_key, x_key, volume_model, params_.D, params_.L));

  graph.add(MassFlowRateFactor(P_key, Ps_key, mdoto_key, mass_rate_model, params_.D,
                               params_.L, params_.mu, params_.epsilon,
                               1.0 / (params_.Rs * params_.T)));

  graph.add(ValveControlFactor(t_key, to_key, tc_key, mdoto_key, mdota_key, mass_rate_model,
                               params_.ct));
...
gtsam::NonlinearFactorGraph PneumaticActuator::sourceFactorGraph(const int t) const {
  gtsam::Key Ps_key = SourcePressureKey(t);
  gtsam::Key ms_key = SourceMassKey(t);
  gtsam::Key vs_key = SourceVolumeKey();

  gtsam::NonlinearFactorGraph graph;
  graph.add(GassLawFactor(Ps_key, vs_key, ms_key, gass_law_model,
                        params_.Rs * params_.T));
  return graph;
}


gtsam::Values PneumaticActuator::computeSourceResult(const int t, const double ms, const double vs) const {
...
gtsam::Values PneumaticActuator::computeResult(const int t, const PriorValues& prior_values,
  ...
  graph.add(gtsam::PriorFactor<double>(q_key, prior_values.q, prior_q_cost_model));
  graph.add(gtsam::PriorFactor<double>(v_key, prior_values.v,
                                        prior_v_cost_model));
  graph.add(gtsam::PriorFactor<double>(m_key, prior_values.m, prior_m_cost_model));

  graph.add(gtsam::PriorFactor<double>(t_key, prior_values.t, prior_time_cost_model));
  graph.add(gtsam::PriorFactor<double>(to_key, prior_values.to, prior_valve_t_cost_model));
  graph.add(gtsam::PriorFactor<double>(tc_key, prior_values.tc, prior_valve_t_cost_model));
  ...
  if (previous_values.exists(PressureKey(params_.j, t-1))) {
    init_values.insert(P_key, previous_values.at(PressureKey(params_.j, t-1)));
    init_values.insert(x_key, previous_values.at(ContractionKey(params_.j, t-1)));
    init_values.insert(f_key, previous_values.at(ForceKey(params_.j, t-1)));
    init_values.insert(torque_key, previous_values.at(TorqueKey(params_.j, t-1)));
    init_values.insert(mdoto_key, previous_values.at(MassRateOpenKey(params_.j, t-1)));
    init_values.insert(mdota_key, previous_values.at(MassRateActualKey(params_.j, t-1)));
    init_values.insert(vo_key, previous_values.at(VolumeKey(params_.j, t-1)));
  }
  else {
    auto factor = ActuatorVolumeFactor(vo_key, x_key, volume_model, params_.D, params_.L);
  ...
  // std::cout << "volume error: " << factor.error(result) << "\t" << "x: " << result.atDouble(x_key) <<"v: " << result.atDouble(vo_key) << "\n";
...
// gtsam::NonlinearFactorGraph PneumaticActuator::actuatorFactorGraph(const int t) const {
//   gtsam::Key t_i_key = StartTimeKey(params_.j);     // time of opening the valve
//   gtsam::Key t_key = TimeKey(t);           // current time
//   gtsam::Key P_i_key = InitPressureKey(params_.j);  // initial pressure
//   gtsam::Key P_key = PressureKey(params_.j, t);   // current pressure
//   gtsam::Key x_key = ContractionKey(params_.j, t);  // contraction length
//   gtsam::Key f_key = ForceKey(params_.j, t);        // force
//   gtsam::Key q_key = JointAngleKey(params_.j, t);   // joint angle
//   gtsam::Key v_key = JointVelKey(params_.j, t);     // joint velocity
//   gtsam::Key torque_key = TorqueKey(params_.j, t);  // torque
...
// gtsam::Values PneumaticActuator::computeResult(const int t, const double angle, const double vel,
