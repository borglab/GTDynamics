...
public:
  using shared_ptr = boost::shared_ptr<RetractParams>;
  ...
  void setProjection(bool _use_basis_keys = false, double _sigma = 1.0, bool _apply_base_retraction = false) {
...
public:
 ...
 protected:
  void checkFeasible(const NonlinearFactorGraph& graph, const Values& values) const;
};
...
protected:
  ...
  std::vector<boost::shared_ptr<ConstVarFactor>> const_var_factors_v_,
...
protected:
  ...
  static void classifyKeys(const CONTAINER& keys, KeySet& q_keys, KeySet& v_keys, KeySet& ad_keys);

  void updatePriors(const Values& values, const KeySet& keys, NonlinearFactorGraph& graph);
};
