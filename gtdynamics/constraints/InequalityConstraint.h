...
public:
  ...
  virtual double toleranceScaledEvaluation(const gtsam::Values &x) const {
    return (*this)(x) / tolerance()(0);
  }
  ...
  virtual gtsam::NoiseModelFactor::shared_ptr
  createPenaltyFactor(const double mu = 1.0) const = 0;

  virtual gtsam::NoiseModelFactor::shared_ptr
  createSmoothPenaltyFactor(const double mu = 1.0,
