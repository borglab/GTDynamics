...
/* ************************************************************************* */
/** A convenient base class for creating your own NoiseModelFactor with 11
 * variables.  To derive from this class, implement evaluateError(). */
template <class VALUE1, class VALUE2, class VALUE3, class VALUE4, class VALUE5,
...
class NoiseModelFactor11 : public gtsam::NoiseModelFactor {
 ...
 protected:
  typedef gtsam::NoiseModelFactor Base;
  typedef NoiseModelFactor11<VALUE1, VALUE2, VALUE3, VALUE4, VALUE5, VALUE6,
 ...
 public:
   ...
   */
  NoiseModelFactor11(const gtsam::SharedNoiseModel &noiseModel, Key j1, Key j2,
                    Key j3, Key j4, Key j5, Key j6, Key j7, Key j8, Key j9,
                    Key j10, Key j11)
      : Base(noiseModel,
             boost::assign::cref_list_of<11>(j1)(j2)(j3)(j4)(j5)(j6)(j7)(j8)(j9)(j10)(j11)) {}
    ...
    if (this->active(x)) {
      if (H)
        return evaluateError(x.at<X1>(keys_[0]), x.at<X2>(keys_[1]),
                             x.at<X3>(keys_[2]), x.at<X4>(keys_[3]),
                             x.at<X5>(keys_[4]), x.at<X6>(keys_[5]),
                             x.at<X7>(keys_[6]), x.at<X8>(keys_[7]),
  ...
  /**
   *  Override this method to finish implementing an 11-way factor.
   *  If any of the optional Matrix reference arguments are specified, it should
   * compute both the function evaluation and its derivative(s) in X1 (and/or
   * X2, X3).
   */
  virtual gtsam::Vector evaluateError(
      const X1 &, const X2 &, const X3 &, const X4 &, const X5 &, const X6 &,
      const X7 &, const X8 &, const X9 &, const X10 &, const X11 &,
...
/** WrenchFactor8 is an eleven-way nonlinear factor which enforces relation
 * between wrenches on this link and the next link*/
class WrenchFactor8
    : public NoiseModelFactor11<gtsam::Vector6, gtsam::Vector6, gtsam::Vector6,
                               gtsam::Vector6, gtsam::Vector6, gtsam::Vector6,
                               gtsam::Vector6, gtsam::Vector6, gtsam::Vector6,
                               gtsam::Vector6, gtsam::Pose3> {
 private:
  typedef WrenchFactor8 This;
  typedef NoiseModelFactor11<gtsam::Vector6, gtsam::Vector6, gtsam::Vector6,
                            gtsam::Vector6, gtsam::Vector6, gtsam::Vector6,
                            gtsam::Vector6, gtsam::Vector6, gtsam::Vector6,
                            gtsam::Vector6, gtsam::Pose3>
      Base;
 ...
 public:
  /** wrench balance factor, common between forward and inverse dynamics.
     ...
     COM frame. Will create factor corresponding to Lynch & Park book:
          - wrench balance, Equation 8.48, page 293
   */
  WrenchFactor8(gtsam::Key twist_key, gtsam::Key twistAccel_key,
                gtsam::Key wrench_key_1, gtsam::Key wrench_key_2,
                gtsam::Key wrench_key_3, gtsam::Key wrench_key_4,
                gtsam::Key wrench_key_5, gtsam::Key wrench_key_6,
                gtsam::Key wrench_key_7, gtsam::Key wrench_key_8,
                gtsam::Key pose_key,
                const gtsam::noiseModel::Base::shared_ptr &cost_model,
                const gtsam::Matrix6 &inertia,
                const boost::optional<gtsam::Vector3> &gravity = boost::none)
      : Base(cost_model, twist_key, twistAccel_key, wrench_key_1, wrench_key_2,
             wrench_key_3, wrench_key_4, wrench_key_5, wrench_key_6,
             wrench_key_7, wrench_key_8, pose_key),
        inertia_(inertia) {
    gravity_ = (gtsam::Vector(3) << 0, 0, 0).finished();
    if (gravity) {
      gravity_ = *gravity;
 ...
 public:
  /** evaluate wrench balance errors
      Keyword argument:
          ...
          wrench_3      -- 3rd wrench on this link
          wrench_4      -- 4th wrench on this link
          wrench_5      -- 5th wrench on this link
          wrench_6      -- 6th wrench on this link
          wrench_7      -- 7th wrench on this link
          wrench_8      -- 8th wrench on this link
  */
  gtsam::Vector evaluateError(
      const gtsam::Vector6 &twist, const gtsam::Vector6 &twistAccel,
      const gtsam::Vector6 &wrench_1, const gtsam::Vector6 &wrench_2,
      const gtsam::Vector6 &wrench_3, const gtsam::Vector6 &wrench_4,
      const gtsam::Vector6 &wrench_5, const gtsam::Vector6 &wrench_6,
      const gtsam::Vector6 &wrench_7, const gtsam::Vector6 &wrench_8,
      const gtsam::Pose3 &pose,
      boost::optional<gtsam::Matrix &> H_twist = boost::none,
      boost::optional<gtsam::Matrix &> H_twistAccel = boost::none,
      boost::optional<gtsam::Matrix &> H_wrench_1 = boost::none,
      ...
      boost::optional<gtsam::Matrix &> H_wrench_3 = boost::none,
      boost::optional<gtsam::Matrix &> H_wrench_4 = boost::none,
      boost::optional<gtsam::Matrix &> H_wrench_5 = boost::none,
      boost::optional<gtsam::Matrix &> H_wrench_6 = boost::none,
      boost::optional<gtsam::Matrix &> H_wrench_7 = boost::none,
      boost::optional<gtsam::Matrix &> H_wrench_8 = boost::none,
      boost::optional<gtsam::Matrix &> H_pose = boost::none) const override {
    // transform gravity from base frame to link COM frame,
    // to use unrotate function, have to convert gravity vector to a point
    gtsam::Point3 gravity_point(gravity_[0], gravity_[1], gravity_[2]);
    ...
    if (H_wrench_5) {
      *H_wrench_5 = -gtsam::I_6x6;
    }
    if (H_wrench_6) {
      *H_wrench_6 = -gtsam::I_6x6;
    }
    if (H_wrench_7) {
      *H_wrench_7 = -gtsam::I_6x6;
    }
    if (H_wrench_8) {
      *H_wrench_8 = -gtsam::I_6x6;
    }
    if (H_pose) {
      *H_pose = -inertia_ * intermediateMatrix * H_unrotate * H_rotation;
    }
 ...
 private:
  ...
  void serialize(ARCHIVE &ar, const unsigned int version) { // NOLINT
    ar &boost::serialization::make_nvp(
        "NoiseModelFactor11", boost::serialization::base_object<Base>(*this));
  }
};
