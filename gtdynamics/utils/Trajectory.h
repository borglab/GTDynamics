...
class Trajectory {
 ...
 public:
   ...
   */
  const vector<gtsam::NonlinearFactorGraph> getTransitionGraphs(
   ...
   */
  const vector<gtsam::Values> getInitTransitionValues(
  ...
  //       double h = GROUND_HEIGHT + std::pow(t_normed, 1.1) * std::pow(1 -
  //       t_normed, 0.7);
  ...
  //     objective_factors.add(gtsam::PriorFactor<Vector6>(
  //         TwistKey(link->getID(), t_f), Vector6::Zero(),
  //         objectives_model_6));
  //     objective_factors.add(gtsam::PriorFactor<Vector6>(
  ...
  //         objective_factors.add(gtsam::PriorFactor<double>(JointAngleKey(joint->getID(),
  //         t), 2.5, dynamics_model_1_2));
  //     }
  ...
  //   240. for (int phase = 0; phase < this->numPhases(); phase++)
  //     objective_factors.add(gtsam::PriorFactor<double>(
  ...
  //       robots, phase_durations, transition_graph_init, dt_des,
