...
//       DH_Link(0, 0, 2, 0, 'R', 1, Point3(-1, 0, 0), Z_3x3, -180, 180, 2,
//       0.15,
//               0.02, 0.12, 0.02, 50, 5),
//       DH_Link(0, 0, 2, 0, 'R', 1, Point3(-1, 0, 0), Z_3x3, -180, 180, 2,
//       0.15,
...
//   auto graph = mp.motionPlanningFactorGraph(robot, pose_goal,
//   Vector::Zero(dof),
//                                             boost::none, gravity,
//                                             boost::none);
...
//   string dir = "./";
//   saveForVisualization(actual_q_trajectory, pose_goal, dof, dir,
//   boost::none);
//   /* +++++++++++++++ output for matlab visualization ++++++++++++++++ */
// #endif
// }
...
//               (Matrix(3, 3) << 0, 0, 0, 0, 0.35, 0, 0, 0, 0).finished(),
//               -180, 180, 2, 0.2, 0.02, 0.2, 0.02, 100, 5),
//       DH_Link(
//           0, 0.4318, 0, 0.0, 'R', 17.40, Point3(-0.3638, 0.006, 0.2275),
//           (Matrix(3, 3) << 0.130, 0, 0, 0, 0.524, 0, 0, 0, 0.539).finished(),
//           -180, 180, 2, 0.2, 0.02, 0.2, 0.02, 100, 5),
//       DH_Link(
//           0, 0.0203, 0.15005, -90, 'R', 4.80, Point3(-0.0203, -0.0141,
//           0.0700), (Matrix(3, 3) << 0.066, 0, 0, 0, 0.086, 0, 0, 0,
//           0.0125).finished(), -180, 180, 2, 0.2, 0.02, 0.2, 0.02, 100, 5),
//       DH_Link(
//           0, 0, 0.4318, +90, 'R', 0.82, Point3(-0.19, 0, 0),
//           (Matrix(3, 3) << 0.0018, 0, 0, 0, 0.0013, 0, 0, 0,
//           0.0018).finished(), -180, 180, 2, 0.2, 0.02, 0.2, 0.02, 100, 5),
//       DH_Link(
//           0, 0.0000, 0.0000, -90, 'R', 0.34, Point3(0, 0, 0),
//           (Matrix(3, 3) << 0.0003, 0, 0, 0, 0.0004, 0, 0, 0,
//           0.0003).finished(), -180, 180, 2, 0.2, 0.02, 0.2, 0.02, 100, 5),
//       DH_Link(0, 0.0000, 0.0000, 0.0, 'R', 0.09, Point3(0, 0, 0.032),
//               (Matrix(3, 3) << 0.00015, 0, 0, 0, 0.00015, 0, 0, 0, 0.00004)
//                   .finished(),
//               -180, 180, 2, 0.2, 0.02, 0.2, 0.02, 100, 5)};
...
TEST(MotionPlanner, urdf_kuka) {
  ...
  vector<URDF_Link> kuka = {
      URDF_Link(Pose3(Rot3(), Point3(0, 0, 0.1575)), Vector3(0, 0, 1), 'R', 4,
                ...
                (Matrix(3, 3) << 0.1, 0, 0, 0, 0.09, 0, 0, 0, 0.02).finished(),
      ...
      URDF_Link(
          ...
          (Matrix(3, 3) << 0.05, 0, 0, 0, 0.018, 0, 0, 0, 0.044).finished(),
          -2.09439510239 * 180 / M_PI, 2.09439510239 * 180 / M_PI, 10, 10, 1),
      URDF_Link(
          Pose3(Rot3::RzRyRx(1.57079632679, 0, 3.14159265359),
                Point3(0, 0.2045, 0)),
          Vector3(0, 0, 1), 'R', 3, Pose3(Rot3(), Point3(0, 0.03, 0.13)),
          (Matrix(3, 3) << 0.08, 0, 0, 0, 0.075, 0, 0, 0, 0.01).finished(),
          -2.96705972839 * 180 / M_PI, 2.96705972839 * 180 / M_PI, 10, 10, 1),
      URDF_Link(
          Pose3(Rot3::RzRyRx(1.57079632679, 0, 0), Point3(0, 0, 0.2155)),
          Vector3(0, 0, 1), 'R', 2.7, Pose3(Rot3(), Point3(0, 0.067, 0.034)),
          (Matrix(3, 3) << 0.03, 0, 0, 0, 0.01, 0, 0, 0, 0.029).finished(),
          -2.09439510239 * 180 / M_PI, 2.09439510239 * 180 / M_PI, 10, 10, 1),
      URDF_Link(
          Pose3(Rot3::RzRyRx(-1.57079632679, 3.14159265359, 0),
                Point3(0, 0.1845, 0)),
          Vector3(0, 0, 1), 'R', 1.7,
          Pose3(Rot3(), Point3(0.0001, 0.021, 0.076)),
          (Matrix(3, 3) << 0.02, 0, 0, 0, 0.018, 0, 0, 0, 0.005).finished(),
          -2.09439510239 * 180 / M_PI, 2.09439510239 * 180 / M_PI, 10, 10, 1),
      URDF_Link(
          Pose3(Rot3::RzRyRx(1.57079632679, 0, 0), Point3(0, 0, 0.2155)),
          Vector3(0, 0, 1), 'R', 1.8, Pose3(Rot3(), Point3(0, 0.0006, 0.0004)),
          (Matrix(3, 3) << 0.005, 0, 0, 0, 0.0036, 0, 0, 0, 0.0047).finished(),
          -2.09439510239 * 180 / M_PI, 2.09439510239 * 180 / M_PI, 10, 10, 1),
      URDF_Link(
          Pose3(Rot3::RzRyRx(-1.57079632679, 3.14159265359, 0),
                Point3(0, 0.081, 0)),
          Vector3(0, 0, 1), 'R', 0.3, Pose3(Rot3(), Point3(0, 0, 0.02)),
          (Matrix(3, 3) << 0.001, 0, 0, 0, 0.001, 0, 0, 0, 0.001).finished(),
          -3.05432619099 * 180 / M_PI, 3.05432619099 * 180 / M_PI, 10, 10, 1)};
  auto robot = Arm<URDF_Link>(
      kuka, gtsam::Pose3(gtsam::Rot3(), gtsam::Point3(-0.1, 0, 0.07)));
  auto poses = robot.comFrames();
  Vector3 gravity;
  gravity << 0, 0, -9.8;
  Vector7 expected_q;
  expected_q << -30, 10, -50, -15, 20, -40, 30;
  expected_q *= M_PI / 180;
  auto expected_T = robot.forwardKinematics(expected_q).back();
  Pose3 pose_goal(expected_T);
  auto dof = robot.numLinks();
  ...
  auto actual_q_trajectory = mp.extractTrajectoryQ(results, dof);
  auto actual_T = robot.forwardKinematics(actual_q_trajectory.back()).back();
  std::cout << "q = " << actual_q_trajectory.back() * 180 / M_PI << std::endl;
  EXPECT(assert_equal(expected_T, actual_T, 1e-3));
  ...
  /* +++++++++++++++ output for matlab visualization ++++++++++++++++ */
  string dir = "/home/mandyxie/Dropbox \(GaTech)/CS/Research/code/dynamics/build/";
  saveForVisualization(actual_q_trajectory, pose_goal, dof, dir, sdf);
  /* +++++++++++++++ output for matlab visualization ++++++++++++++++ */
#endif
}
