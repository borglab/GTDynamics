 ...
 * @brief test forward dynamics of robotic manipulator with closed kinematic loop
 * @Author: Yetong Zhang
 */
...
TEST(JumpingRobot, forward_dynamics)
{
        ...
        double acc = 
                (torque3 - (0.5*example::m1 + 1.5*example::m2 + 1.0*example::m3)*9.8*example::l*std::sin(theta))/(
                std::pow(example::l,2) * 
                (1.0/4*example::m1 + (1.0/4+2*std::pow(std::sin(theta),2))*example::m2 + 2*std::pow(std::sin(theta),2)*example::m3)
