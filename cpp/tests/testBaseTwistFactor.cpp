...
TEST(BaseTwistFactor, error)
{
    ...
    expected_H = numericalDerivative11(boost::function<Vector6(const Vector6 &)>(
...
TEST(BaseTwistFactor, optimaization)
{
    ...
    Key vKey = Symbol('V', 0);
    Vector6 base_twist;
    base_twist = (Vector(6) << 0, 0, 1, 0, 0, 0).finished();
    Vector base_twist_init;
    base_twist_init = (Vector(6) << 0, 0, 0, 0, 0, 0).finished();
