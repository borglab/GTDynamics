/**
 * @file  test_serial_link.cpp
 * @brief test serial link
...
{
    return Pose3(Rot3::Rz(theta), Point3()) * Pose3(Rot3(), Point3(2, 0, 0));
}

// Unit tests for dh link RR
TEST(SerialLink, DH_RR)
{
    vector<DH_Link> dh_rr = {DH_Link(0, 0, 2, 0, 'R', 1, Point3(-1, 0, 0), Vector3(0, 0, 0), -180, 180, 20),
                             DH_Link(0, 0, 2, 0, 'R', 1, Point3(-1, 0, 0), Vector3(0, 0, 0), -180, 180, 20)};
    ...
    Vector6 expected_spatial_screwAxes_1 = unit_twist(Vector3(0, 0, 1), Vector3(0, 0, 0));
    Vector6 expected_spatial_screwAxes_2 = unit_twist(Vector3(0, 0, 1), Vector3(2, 0, 0));
    ...
    Vector actual_joint_coordinates = robot.inverseKinematics(goal_pose, joint_coordinates_init);
    Vector expected_joint_coordinates = (Vector(2) << M_PI / 4.0, 0).finished();
    EXPECT(assert_equal(actual_joint_coordinates, expected_joint_coordinates, 1e-6));
    /* ================================================================= */
...
TEST(SerialLink, DH_PUMA)
{
    vector<DH_Link> dh_puma = {
        DH_Link(0, 0.0000, 0.0000, +90, 'R', 0, Point3(0, 0, 0), Vector3(0, 0.35, 0), -180, 180, 20),
        DH_Link(0, 0.4318, 0, 0.0, 'R', 17.40, Point3(-0.3638, 0.006, 0.2275), Vector3(0.130, 0.524, 0.539), -180, 180, 20),
        DH_Link(0, 0.0203, 0.15005, -90, 'R', 4.80, Point3(-0.0203, -0.0141, 0.0700), Vector3(0.066, 0.086, 0.0125), -180, 180, 20),
        DH_Link(0, 0, 0.4318, +90, 'R', 0.82, Point3(0, 0.19, 0), Vector3(0.0018, 0.0013, 0.0018), -180, 180, 20),
        DH_Link(0, 0.0000, 0.0000, -90, 'R', 0.34, Point3(0, 0, 0), Vector3(0.0003, 0.0004, 0.0003), -180, 180, 20),
        DH_Link(0, 0.0000, 0.0000, 0.0, 'R', 0.09, Point3(0, 0, 0.032), Vector3(0.00015, 0.00015, 0.00004), -180, 180, 20)};

    // Create Puma robot.
    auto robot = SerialLink<DH_Link>(dh_puma, Pose3(), Pose3());
    int N = robot.numLinks();
    vector<Pose3> frames;
    ...
    expected_torques << 2.14242560e+00, -4.72874900e+01, 1.37677604e+01, 2.15162000e-01, 1.45261716e-03, 7.67944871e-05;
    expected_joint_accelerations << 0.174533, 0.349066, 0.523599, 0.698132, 0.872665, 1.047198;
    torques << 2.14242560e+00, -4.72874900e+01, 1.37677604e+01, 2.15162000e-01, 1.45261716e-03, 7.67944871e-05;
    joint_accelerations << 0.174533, 0.349066, 0.523599, 0.698132, 0.872665, 1.047198;
    base_twist_accel << 0, 0, 0, 0, 0, 0;
    Vector3 gravity(0, 0, -9.8);
    factor_graph = robot.forwardDynamicsFactorGraph(
        joint_angles, radians(joint_velocities), torques,
        base_twist_accel, external_wrench, gravity);
    EXPECT(assert_equal(factor_graph.size(), 1 + N * 3 + 1));
...
TEST(SerialLink, URDF_RR)
{
    vector<URDF_Link> urdf_rr = {URDF_Link(Pose3(Rot3(), Point3(2, 0, 0)), Vector3(0, 0, 1), 'R',
                                           1, Pose3(Rot3(), Point3(1, 0, 0)), Vector3(0, 0, 0), -180, 180, 20),
                                 URDF_Link(Pose3(Rot3(), Point3(2, 0, 0)), Vector3(0, 0, 1), 'R',
                                           1, Pose3(Rot3(), Point3(1, 0, 0)), Vector3(0, 0, 0), -180, 180, 20)};

    // The joint screw axis, in the COM frame, is the same for both joints
    auto AXIS = unit_twist(Vector3(0, 0, 1), Vector3(-1, 0, 0));
    auto robot = SerialLink<URDF_Link>(urdf_rr, Pose3(), Pose3(Rot3(), Point3(2, 0, 0)));
    int N = robot.numLinks();
