...
DH_Link dh_link = DH_Link(0, 0, 2, 0, 'R', 1, Point3(-1, 0, 0), Vector3(0, 1/6.0, 1/6.0), -5, 10, 2);
...
TEST(Link, forward_factors)
{
    ...
    GaussianFactorGraph factors = dh_link.forwardFactors(2, jTi, v2, twist_2, torque_2, kTj);
...
TEST(Link, inverse_factors)
{
    ...
    Pose3 jTi = Pose3(Rot3(), Point3(-2, 0, 0));
    Pose3 kTj = Pose3(Rot3(), Point3(-2, 0, 0));
    GaussianFactorGraph factors = dh_link.inverseFactors(2, jTi, v2, twist_2, acceleration_2, kTj);
    EXPECT(assert_equal(factors.size(), 3));
