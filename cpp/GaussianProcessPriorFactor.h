...
class GaussianProcessPriorFactor
  ...
  */
  ...
  gtsam::Vector evaluateError(
    ...
    // Jacobians
    if (H_q1) *H_q1 = (gtsam::Matrix(3, 1) << 1, 0, 0).finished();
    if (H_qVel1) *H_qVel1 = (gtsam::Matrix(3, 1) << delta_t_, 1, 0).finished();
    if (H_qAccel1)
      *H_qAccel1 =
          (gtsam::Matrix(3, 1) << 0.5 * delta_t_ * delta_t_, delta_t_, 1)
              .finished();
