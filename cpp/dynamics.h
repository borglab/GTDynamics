...
virtual class Link {
  ...
  Link(size_t joint_type, double mass, const gtsam::Pose3& center_of_mass,
       Matrix inertia, Vector screwAxis, double joint_lower_limit,
       double joint_upper_limit, double joint_limit_threshold,
       double velocity_limit, double velocity_limit_threshold,
       double acceleration_limit, double acceleration_limit_threshold,
  ...
  static gtsam::JacobianFactor* BaseTwistAccelFactor(Vector base_twist_accel);

  static gtsam::JacobianFactor* ToolWrenchFactor(int N, Vector external_wrench);

  gtsam::JacobianFactor* twistFactor(int j, const gtsam::Pose3& jTi,
                                     double joint_vel_j) const;

  gtsam::JacobianFactor* wrenchFactor(int j, Vector twist_j,
  ...
  gtsam::GaussianFactorGraph forwardFactors(int j, const gtsam::Pose3& jTi,
                                            double joint_vel_j, Vector twist_j,
                                            double torque_j,
                                            const gtsam::Pose3& kTj) const;
  ...
  gtsam::GaussianFactorGraph inverseFactors(int j, const gtsam::Pose3& jTi,
                                            double joint_vel_j, Vector twist_j,
...
virtual class DhLink : manipulator::Link {
  ...
  DhLink(double theta, double d, double a, double alpha, size_t joint_type,
         double mass, const gtsam::Point3& center_of_mass, Matrix inertia,
...
virtual class UrdfLink : manipulator::Link {
  UrdfLink(const gtsam::Pose3& origin, Vector axis, size_t joint_type,
           double mass, const gtsam::Pose3& center_of_mass, Matrix inertia,
           double joint_lower_limit, double joint_upper_limit,
           double joint_limit_threshold, double velocity_limit,
           double velocity_limit_threshold, double acceleration_limit,
           double acceleration_limit_threshold, double torque_limit,
...
virtual class JointLimitVectorFactor : gtsam::NoiseModelFactor {
  JointLimitVectorFactor(gtsam::Key pose_key,
                         ...
                         Vector lower_limits, Vector upper_limits,
...
virtual class Arm {
  ...
  // //       Vector q,
  // //       boost::optional<std::vector<gtsam::Matrix>& > J = boost::none)
  // const;
  ...
  gtsam::GaussianFactorGraph forwardDynamicsFactorGraph(
      Vector q, Vector joint_velocities, Vector torques,
      Vector base_twist_accel, Vector external_wrench) const;
  ...
  gtsam::GaussianFactorGraph inverseDynamicsFactorGraph(
      Vector q, Vector joint_velocities, Vector joint_accelerations,
      Vector base_twist_accel, Vector external_wrench) const;
  ...
  Vector extractJointAcceleraions(const gtsam::VectorValues& result) const;
  ...
  Vector forwardDynamics(Vector q, Vector joint_velocities, Vector torques,
                         Vector base_twist_accel, Vector external_wrench) const;
  ...
  Vector inverseDynamics(Vector q, Vector joint_velocities,
                         Vector joint_accelerations, Vector base_twist_accel,
  ...
  manipulator::PoseGoalFactor poseGoalFactor(
  ...
  Vector inverseKinematics(const gtsam::Pose3& goal_pose, Vector init_q) const;
};
...
virtual class BaseTwistFactor : gtsam::NoiseModelFactor {
  BaseTwistFactor(gtsam::Key twist_key_0,
                  const gtsam::noiseModel::Base* cost_model, Vector base_twist);
};

#include <GaussianProcessPriorFactor.h>
virtual class GaussianProcessPriorFactor : gtsam::NoiseModelFactor {
  GaussianProcessPriorFactor(gtsam::Key q_key1, gtsam::Key qVel_key1,
                             gtsam::Key qAccel_key1, gtsam::Key q_key2,
                             gtsam::Key qVel_key2, gtsam::Key qAccel_key2,
                             const gtsam::noiseModel::Gaussian* Qc_model,
                             double delta_t);
};

#include <GaussianProcessPriorPose3Factor.h>
virtual class GaussianProcessPriorPose3Factor : gtsam::NoiseModelFactor {
  GaussianProcessPriorPose3Factor(gtsam::Key p1_key, gtsam::Key v1_key,
                                  gtsam::Key vdot1_key, gtsam::Key p2_key,
                                  gtsam::Key v2_key, gtsam::Key vdot2_key,
                                  const gtsam::noiseModel::Gaussian* Qc_model,
                                  double delta_t);
};
...
virtual class JointLimitFactor : gtsam::NoiseModelFactor {
  JointLimitFactor(gtsam::Key q_key, const gtsam::noiseModel::Base* cost_model,
...
virtual class PoseFactor : gtsam::NoiseModelFactor {
  PoseFactor(gtsam::Key pose_key_i, gtsam::Key pose_key_j, gtsam::Key q_key,
             const gtsam::noiseModel::Base* cost_model, const gtsam::Pose3& jMi,
             Vector screw_axis);
};
...
class SphereLink {
  // SphereLink(double radius, const std::vector<gtsam::Point3>&
  // sphere_centers);
};

#include <ToolPoseFactor.h>
virtual class ToolPoseFactor : gtsam::NoiseModelFactor {
  ToolPoseFactor(gtsam::Key pose_key, const gtsam::noiseModel::Base* cost_model,
...
virtual class TorqueFactor : gtsam::NoiseModelFactor {
  TorqueFactor(gtsam::Key wrench_key, gtsam::Key torque_key,
               const gtsam::noiseModel::Base* cost_model, Vector screw_axis);
};
...
virtual class TwistFactor : gtsam::NoiseModelFactor {
  TwistFactor(gtsam::Key twistI_key, gtsam::Key twistJ_key, gtsam::Key q_key,
              gtsam::Key qVel_key, const gtsam::noiseModel::Base* cost_model,
              const gtsam::Pose3& jMi, Vector screw_axis);
};
...
virtual class WrenchFactor : gtsam::NoiseModelFactor {
  WrenchFactor(gtsam::Key twist_key, gtsam::Key twistAccel_key,
               ...
               const gtsam::noiseModel::Base* cost_model,
               const gtsam::Pose3& kMj, Matrix inertia, Vector screw_axis);
