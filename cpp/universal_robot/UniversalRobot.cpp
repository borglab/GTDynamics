...
RobotJointPair extract_structure_from_sdf(
  ...
  for (uint i = 0; i < sdf.LinkCount(); i++) {
    robot::RobotLinkSharedPtr l_i = std::make_shared<robot::RobotLink>(
      *sdf.LinkByIndex(i));
  ...
  for (uint i = 0; i < sdf.JointCount(); i++) {
    ...
    robot::RobotJointSharedPtr joint_strong = std::make_shared<robot::RobotJoint>(
      robot::RobotJoint(
...
UniversalRobot::UniversalRobot(const std::string file_path, std::string model_name) : 
...
void UniversalRobot::printRobot() const {
    for (const auto& link: link_bodies_) {
        std::cout<<link->name() << ":\n";
        std::cout<<"\tlink pose: " << link->Twl().rotation().rpy().transpose() << ", " << link->Twl().translation() << "\n";
        std::cout<<"\tcom pose: " << link->Twcom().rotation().rpy().transpose() << ", " << link->Twcom().translation() << "\n";
        std::cout << "\tjoints: ";
