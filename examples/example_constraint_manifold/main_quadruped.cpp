...
void TrajectoryOptimization(size_t numSteps,
  ...
  auto initValues = vision60.getInitValuesTrajectory(
      numSteps, dt, basePoseInit, desPoses, desPosesT);
  NonlinearFactorGraph collocationCosts = vision60.collocationCosts(numSteps, dt);
  ...
  runner.setMoptFactory([&](BenchmarkMethod) {
    auto moptParams =
        ConstrainedOptBenchmark::DefaultMoptParamsSV(vision60.getBasisKeyFunc());
...
int main(int argc, char **argv) {
  try {
    ...
    defaults.defaultMethods = {BenchmarkMethod::SOFT, BenchmarkMethod::CM_I};
    ...
    std::cout << "Methods: " << "\n";
    for (BenchmarkMethod method : parsed.runOptions.methods) {
