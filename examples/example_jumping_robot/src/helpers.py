...
def read_pressure(path_data: str, prefix_data: str) -> (np.array, np.array):
    ''' Import experimental pressure data '''
    file_pressures = os.path.join(path_data, prefix_data, (prefix_data + ' pres_filt.txt'))
...
def read_marker_pix(path_data: str, prefix_data: str, dim_camera) -> (np.array, np.array):
    ''' Import marker pixel location measurements ''' 
    file_vid_time = os.path.join(path_data, prefix_data, (prefix_data + ' vid_time.txt'))
    time = np.loadtxt(file_vid_time, dtype='float', comments='#', delimiter=',')
    time = time - time[0] # zero start time

    file_marker_pix = os.path.join(path_data, prefix_data, (prefix_data + ' marker_pix.txt'))
...
time_exp, pressure_exp = read_pressure(path_data, prefix_data)
time_vid, pix_vid = read_marker_pix(path_data, prefix_data, dim_camera)
