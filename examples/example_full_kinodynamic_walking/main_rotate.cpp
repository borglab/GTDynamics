...
int main(int argc, char** argv) {
  ...
  vector<CPs> phase_cps =   {p0, p1, p0, p2, p0, p3, p0, p4, p0, p1, p0, p2, p0, p3, p0, p4};
  vector<CPs> trans_cps =   {  p1, p1, p2, p2, p3, p3, p4, p4, p1, p1, p2, p2, p3, p3, p4  };
  vector<int> phase_steps = {50, 60, 50, 60, 50, 60, 50, 60, 50, 60, 50, 60, 50, 60, 50, 60};
  ...
  for (auto&& link : links)
    centroid += (link_map[link]->wTcom() * Pose3(Rot3(), c0.contact_point)).translation();
  centroid = centroid / 4;

  std::map<string, double> centroid_contact_dist;
  std::map<string, double> prev_theta;
  for (auto&& link : links) {
    auto cp = (link_map[link]->wTcom() * Pose3(Rot3(), c0.contact_point)).translation();
  ...
  for (int p = 0; p < phase_cps.size(); p++) {
    ...
    for (int t = t_p_i; t <= t_p_f; t++) {
      ...
      for (auto&& pcl : phase_contact_links) {
        ...
        auto new_cp = Point3(
          centroid.x() + centroid_contact_dist[pcl] * std::cos(prev_theta[pcl]),
          centroid.y() + centroid_contact_dist[pcl] * std::sin(prev_theta[pcl]),
          GROUND_HEIGHT - 0.03);

        objective_factors.add(gtdynamics::PointGoalFactor(
          PoseKey(link_map[pcl]->getID(), t), objectives_model_3,
          Pose3(Rot3(), c0.contact_point), new_cp));
      }
      ...
      for (auto&& psl : phase_swing_links) {
        auto new_cp = Point3(
          centroid.x() + centroid_contact_dist[psl] * std::cos(prev_theta[psl]),
          centroid.y() + centroid_contact_dist[psl] * std::sin(prev_theta[psl]),
          h);
  ...
  auto base_pose_goal = Pose3(Rot3::Rz(theta_inc * phase_cps.size() / 8.), Point3(0, 0, 0.13));
  auto base_pose_model = gtsam::noiseModel::Diagonal::Sigmas(
    (Vector(6) << sigma_objectives * 3, sigma_objectives * 3,
                  sigma_objectives * 100, 10000, sigma_objectives,
