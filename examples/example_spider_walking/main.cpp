...
int main(int argc, char** argv) {
  ...
  // vector<CPs> trans_cps =   {t01, t01, t05, t05, t01, t01, t05, t05, t01, t01, t05, t05};
  // vector<int> phase_steps = {50, 60, 50, 60, 50, 60, 50, 60, 50, 60, 50, 60, 50};
  ...
  // Define the cumulative phase steps.
    ...
    for (int p = 0; p < phase_cps.size(); p++) {
        ...
        for (int t = t_p_i; t <= t_p_f; t++) {
            ...
            //------------> What if contact links work for only those with 4 phases?
    ...
    for (auto&& joint : spider.joints()) {
        objective_factors.add(gtsam::PriorFactor<double>(JointAngleKey(joint->getID(), 0), 0, dynamics_model_1));
        // if (joint->name().find("hip_") == 0){
        //     objective_factors.add(gtsam::PriorFactor<double>(JointAngleKey(joint->getID(), 0), -0.6, dynamics_model_1)); 
        //   }
        // else if (joint->name().find("hip2") == 0){
        //     objective_factors.add(gtsam::PriorFactor<double>(JointAngleKey(joint->getID(), 0), 0, dynamics_model_1));
        //   }
        // else if (joint->name().find("knee") == 0){
        //     objective_factors.add(gtsam::PriorFactor<double>(JointAngleKey(joint->getID(), 0), -0.3, dynamics_model_1));
        //   }
        //   else{
        //     objective_factors.add(gtsam::PriorFactor<double>(JointAngleKey(joint->getID(), 0), -0.9, dynamics_model_1));
        //   }
            // std::cout << joint->getID() <<std::endl;
