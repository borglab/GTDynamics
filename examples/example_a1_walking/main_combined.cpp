...
int CombinedRun(bool add_mass_to_body) {
  ...
  if (add_mass_to_body) {
    ...
    std::cout << "robot body mass set to "
              << robot_massless.link("trunk")->mass() << std::endl;
  }
  ...
  OptimizerSetting opt(sigma_dynamics, sigma_dynamics, sigma_dynamics,
                       sigma_dynamics);
  ...
  auto graph_DG_mass = trajectory.multiPhaseFactorGraph(
      robot, graph_mass_builder, collocation, mu);
  std::cout << "graph_DG_massless" << std::endl;
  auto graph_DG_massless = trajectory.multiPhaseFactorGraph(
      robot_massless, graph_massless_builder, collocation, mu);
  std::cout << "graph_CDG_massless" << std::endl;
  auto graph_CDG_massless = trajectory.multiPhaseFactorGraph(
      robot_massless, chain_graph_massless_builder, collocation, mu);
  ...
  gtsam::NonlinearFactorGraph objectives = trajectory.contactPointObjectives(
      robot, Isotropic::Sigma(3, 8e-6), step, ground_height);
  ...
  for (auto&& joint : robot.joints())
    if (joint->name().find("lower") < 100)
      for (int k = 0; k <= K; k++) {
        ...
        // std::endl;
        objectives.add(
            JointObjectives(joint->id(), k).angle(-1.4, prior_model_lower));
  ...
  for (auto&& joint : robot.joints())
    if (joint->name().find("upper") < 100)
      for (int k = 0; k <= K; k++) {
        // std::cout << joint->name() << joint->name().find("hip") << std::endl;
        objectives.add(
            JointObjectives(joint->id(), k).angle(0.9, prior_model_upper));
        //.velocity(0.0, prior_model_va).acceleration(0.0, prior_model_va));
      }
  ...
  trajectory.addBoundaryConditions(
      &boundary_objectives_DG, robot, dynamics_model_6, dynamics_model_6,
      objectives_model_6, objectives_model_1, objectives_model_1);
  ...
  gtsam::Values init_vals_DG = trajectory.multiPhaseInitialValues(
      robot, initializer, gaussian_noise, desired_dt);
  gtsam::Values init_vals_CDG = trajectory.multiPhaseInitialValues(
      robot_massless, chain_initializer, gaussian_noise, desired_dt);
  ...
  std::cout << "graph_DG_mass keys size = " << graph_DG_mass.keys().size()
            << std::endl;
  std::cout << "graph_DG_mass init vals size = " << init_vals_DG.size()
            << std::endl;

  // Optimize!
  gtsam::LevenbergMarquardtOptimizer optimizer_DG_mass(graph_DG_mass,
                                                       init_vals_DG, params);
  ...
  std::cout << "graph_DG_massless size = " << graph_DG_massless.size()
            << std::endl;
  std::cout << "graph_DG_massless keys size = "
            << graph_DG_massless.keys().size() << std::endl;
  std::cout << "graph_DG_massless init vals size = " << init_vals_DG.size()
            << std::endl;

  // Optimize!
  gtsam::LevenbergMarquardtOptimizer optimizer_DG_massless(
      graph_DG_massless, init_vals_DG, params);
  ...
  // gtsam::tictoc_print_(); //need to build gtsam with timing for this
  //  Write results to traj file
  trajectory.writeToFile(robot_massless, "a1_traj_DG_massless.csv",
                         results_DG_massless);

  gttic_(optimization3);

  std::cout << "graph_CDG_massless size = " << graph_CDG_massless.size()
            << std::endl;
  std::cout << "graph_CDG_massless keys size = "
            << graph_CDG_massless.keys().size() << std::endl;
  std::cout << "graph_CDG_massless init vals size = " << init_vals_CDG.size()
            << std::endl;

  // Optimize!
  gtsam::LevenbergMarquardtOptimizer optimizer_CDG_massless(
      graph_CDG_massless, init_vals_CDG, params);
  ...
  // gtsam::tictoc_print_(); //need to build gtsam with timing for this
  //  Write results to traj file
  trajectory.writeToFile(robot_massless, "a1_traj_CDG_massless.csv",
                         results_CDG_massless);
  ...
  for (int i = 0; i < K; ++i) {
    // std::cout << results_DG_massless.at<gtsam::Vector6>(WrenchKey(0,0,i)) <<
    // std::endl; std::cout <<
    // results_CDG_massless.at<gtsam::Vector6>(WrenchKey(0,0,i)) << std::endl;
    gtsam::Pose3 pose1 =
        results_DG_mass.at<gtsam::Pose3>(PoseKey(link_to_test, i));
    gtsam::Pose3 pose2 =
        results_DG_massless.at<gtsam::Pose3>(PoseKey(link_to_test, i));
    gtsam::Pose3 pose3 =
        results_CDG_massless.at<gtsam::Pose3>(PoseKey(link_to_test, i));
    FileLocations << pose1.translation().x() << "," << pose1.translation().y()
                  << "," << pose1.translation().z() << ","
                  << pose2.translation().x() << "," << pose2.translation().y()
                  << "," << pose2.translation().z() << ","
                  << pose3.translation().x() << "," << pose3.translation().y()
                  << "," << pose3.translation().z() << "\n";
    gtsam::Vector6 wrench1 =
        results_DG_mass.at<gtsam::Vector6>(WrenchKey(0, joint_to_test, i));
    gtsam::Vector6 wrench2 =
        results_DG_massless.at<gtsam::Vector6>(WrenchKey(0, joint_to_test, i));
    gtsam::Vector6 wrench3 =
        results_CDG_massless.at<gtsam::Vector6>(WrenchKey(0, joint_to_test, i));
    FileWrenchesBody << wrench1[0] << "," << wrench1[1] << "," << wrench1[2]
                     << "," << wrench1[3] << "," << wrench1[4] << ","
                     << wrench1[5] << "," << wrench2[0] << "," << wrench2[1]
                     << "," << wrench2[2] << "," << wrench2[3] << ","
                     << wrench2[4] << "," << wrench2[5] << "," << wrench3[0]
                     << "," << wrench3[1] << "," << wrench3[2] << ","
                     << wrench3[3] << "," << wrench3[4] << "," << wrench3[5]
                     << "\n";
    ...
    gtsam::Vector6 wrench6 =
        results_CDG_massless.at<gtsam::Vector6>(WrenchKey(0, 0, i));

    double torque0_cdg_massless = (-1) * wrench6.transpose() * T0.AdjointMap() *
                                  robot.joint("FL_hip_joint")->cScrewAxis();
    double torque1_cdg_massless = (-1) * wrench6.transpose() *
                                  (T0 * T1).AdjointMap() *
                                  robot.joint("FL_upper_joint")->cScrewAxis();
    double torque2_cdg_massless = (-1) * wrench6.transpose() *
                                  (T0 * T1 * T2).AdjointMap() *
                                  robot.joint("FL_lower_joint")->cScrewAxis();

    FileTorques << torque0_dg_mass << ',' << torque1_dg_mass << ','
                << torque2_dg_mass << ',' << torque0_dg_massless << ','
                << torque1_dg_massless << ',' << torque2_dg_massless << ','
                << torque0_cdg_massless << ',' << torque1_cdg_massless << ','
                << torque2_cdg_massless << '\n';

    if (i < 13 || i > 25) continue;

    gtsam::Vector6 wrench4 =
        results_DG_mass.at<gtsam::Vector6>(ContactWrenchKey(3, 0, i))
            .transpose() *
        T3.AdjointMap();
    gtsam::Vector6 wrench5 =
        results_DG_massless.at<gtsam::Vector6>(ContactWrenchKey(3, 0, i))
            .transpose() *
        T3.AdjointMap();
    gtsam::Vector6 wrench7 = wrench6.transpose() * T_body_contact.AdjointMap();
    FileWrenchesGround << wrench4[0] << "," << wrench4[1] << "," << wrench4[2]
                       << "," << wrench4[3] << "," << wrench4[4] << ","
                       << wrench4[5] << "," << wrench5[0] << "," << wrench5[1]
                       << "," << wrench5[2] << "," << wrench5[3] << ","
                       << wrench5[4] << "," << wrench5[5] << "," << wrench7[0]
                       << "," << wrench7[1] << "," << wrench7[2] << ","
                       << wrench7[3] << "," << wrench7[4] << "," << wrench7[5]
                       << "\n";
  }
  FileLocations.close();
