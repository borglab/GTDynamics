...
Trajectory getTrajectory(const Robot& robot, size_t repeat) {
  ...
  std::vector<size_t> phase_lengths = {25,25,25,25};
...
int oldGraph(){
  ...
  gtsam::NonlinearFactorGraph objectives =
      trajectory.contactPointObjectives(robot, Isotropic::Sigma(3, 1e-3), step, ground_height);
  ...
  for (int k = 0; k <= K; k++) {
    objectives.add(
        LinkObjectives(base_link->id(), k)
            .pose(Pose3(Rot3(), Point3(0, 0, 0.4)), Isotropic::Sigma(6, 1e-6))
  ...
  /*for (auto&& joint : robot.joints())
    if (joint->name().find("upper") < 100)
      for (int k = 0; k <= K; k++) {
        //std::cout << joint->name() << joint->name().find("lower") << std::endl;
        objectives.add(JointObjectives(joint->id(), k).torque(0.0, prior_model_torques));
      }
  ...
  for (auto&& joint : robot.joints())
    if (joint->name().find("hip") < 100)
      for (int k = 0; k <= K; k++) {
        ...
        objectives.add(JointObjectives(joint->id(), k).angle(0.0, prior_model_angles));
      }
  ...
  for (auto&& joint : robot.joints())
    if (joint->name().find("upper") < 100)
      for (int k = 0; k <= K; k++) {
        //std::cout << joint->name() << joint->name().find("hip") << std::endl;
        objectives.add(JointObjectives(joint->id(), k).angle(0.7, prior_model_angles));
      }
 ...
 Initializer initializer;
  ...
  trajectory.writeToFile(robot, "a1_traj_old.csv", results);
...
int newGraph(){
  ...
  ChainDynamicsGraph graph_builder(robot, opt, 1e-2, 1e-1, 1e-3, gravity);
  ...
  for (auto &&link : robot.links()) {
    ...
    objectives.add(LinkObjectives(link->id(), 0)
                   .pose(link->bMcom(), Isotropic::Sigma(6, 1e-6)));
  }
  // initial and end joint velocity and accelerations should be zero
  objectives.add(JointsAtRestObjectives(robot, objectives_model_1,
  ...
  gtsam::tictoc_print_(); //need to build gtsam with timing for this
  // Write results to traj file
  trajectory.writeToFile(robot, "a1_traj_new.csv", results);
